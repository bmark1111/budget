...
class Cli {
	 ...
	 *
	 * @return mixed Specified item from CI's superobject.
	 */
	 ...
	 * @return object
	 */
	public function status($message, $status = TRUE)
	{
		$status = $status ? '[SUCCESS]' : '[FAILED]';
		return $this->write_compatible($message . '... ' . $status);
	}
	 ...
	 * @return object
	 */
	public function info($message, $new_line = FALSE)
	{
		return $this->write_compatible($message, $new_line, 'info');
	}
	 ...
	 * @return object
	 */
	public function err($message, $new_line = FALSE)
	{
		return $this->write_compatible($message, $new_line, 'error');
	}
	...
	public function sleep($seconds = 0, $countdown = FALSE)
	{
		...
		{
			echo "Waiting for $seconds second(s)...<br />";
		}
	...
	public function write_compatible($message, $new_line = FALSE, $type = 'info')
	{
		...
		is_array($message) AND $message = implode(
			$this->input->is_cli_request() ? PHP_EOL : '<br />',
			$message
		);
