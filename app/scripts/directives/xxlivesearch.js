app.directive("liveSearch", ["$compile", "$timeout", function ($compile, $timeout) {
	return {
		restrict: 'E',
		replace: true,
		scope: {
...
console.log(item)
			...
			element[0].onkeyup = function (e) {
				...
				timeout = $timeout(function () {
					scope.liveSearchCallback.call(null, { query: search_string });
//					var results = [];
...
//			var itemTemplate = element.attr("live-search-item-template") || "{{result.name}}";
			var template = "<ul ng-show='visible' ng-style=\"{'top':top,'left':left,'width':width}\" class='searchresultspopup'>" +
